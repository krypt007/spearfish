...
import { MUTATION_CREATE_CATEGORY_TRANSACTION } from "../graphql/mutations";
...
const DateInput = ({ name, control, label }) => {
  return (
...
const NewTransaction = ({ isOpen, closeModal }) => {
  const { handleSubmit, reset, control } = useForm();
  const { budget_id, dial_id } = useParams();
  const [createCategoryTransaction] = useMutation(MUTATION_CREATE_CATEGORY_TRANSACTION);
  ...
  return (
    <Modal className='new-transaction-container' isOpen={isOpen} closeModal={closeModal}>
      <Notification severity={notificationError ? 'error' : 'success'} show={notificationOpen}
        setShow={setNotificationOpen} description={notificationError || 'Added successfully'} />
      <div>
        <div className="mb-20">
          <p className="fw-bold fs-1-2 text-gradient">Add sub dial</p>
          <p className="fs-0-7">Create amounts used in the current dial</p>
        </div>
